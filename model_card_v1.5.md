...
roughly 24,100 pairs with a full negative set. That count varies between runs: four recorded
ablation runs mining from the same first-stage checkpoint gave between 24,096 and 24,101, and the
release run's own count was not recorded. The released checkpoint trains on those pairs with a
cached multiple-negatives ranking loss at batch 128, wrapped in a Matryoshka loss over 768, 512,
256, 128 and 64 dimensions, for three epochs at learning rate 2e-5 on one T4.
